...
def test_cmd_key_set_env(mock_ask, chat, mock_console):
    with patch.dict("os.environ", {}):
        ...
        with patch("siyarix.credential_store.CredentialStore", side_effect=Exception("No store")):
            ...
            mock_console.print.assert_any_call("[dim]Tip: Key will only last for this session. Install cryptography for persistent storage: pip install cryptography[/dim]")
...
def test_cmd_report_no_engine(chat, mock_console):
    ...
    mock_console.print.assert_any_call("[yellow]Report generation is not available in this version[/yellow]")
...
def test_show_key_status(chat, mock_console):
    with patch("siyarix.providers.ProviderManager.get_instance") as mock_pm, \
        ...
        mock_registry.get_profile.side_effect = lambda p: profile_openai if p == "openai" else profile_anthropic
...
def test_cmd_key_rotate(mock_ask, chat, mock_console):
    with patch("siyarix.credential_store.CredentialStore") as mock_cs:
        ...
        mock_console.print.assert_any_call("[green]✓ Master encryption key rotated successfully[/green]")
