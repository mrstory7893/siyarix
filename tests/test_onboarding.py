...
def mock_rich_inputs():
    with patch("siyarix.onboarding.Prompt.ask", return_value="1") as m_prompt, \
...
async def test_onboarding_full_run(mock_get_config, mock_subproc, mock_which, mock_settings, mock_console, tmp_path):
    ...
    with patch("siyarix.onboarding.Prompt.ask", side_effect=["c", "5", "1", "a", "1", "1", "1", "1", "1"]):
