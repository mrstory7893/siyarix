...
async def test_e2e_simulation_network_scan_and_enum():
    ...
    goal = AgentGoal(description="Scan 192.168.1.1, if port 80 open, enumerate directories.", target="192.168.1.1")
    ...
    async def mock_execute_step(step, context):
        if step.tool == "nmap":
            ...
            step.result = {"stdout": "80/tcp open http", "findings": [{"port": 80, "service": "http"}]}
            # Add to KG
            ...
            agent._knowledge_graph.add_node(NodeType.URL, "path_admin", properties={"path": "/admin"})
        return step
    ...
    with patch.object(agent._planner_autonomous, "plan", new_callable=AsyncMock) as mock_plan:
        ...
        with patch.object(agent._executor_autonomous, "execute_plan", side_effect=mock_execute_plan):
            with patch.object(agent._validator, "validate_plan", new_callable=AsyncMock) as mock_val:
...
async def test_e2e_simulation_failure_and_recovery():
    ...
    async def mock_execute_plan(p, *args, **kwargs):
        ...
        p.status = PlanStatus.COMPLETED if p.steps[0].status != StepStatus.FAILED else PlanStatus.FAILED
        return p
            ...
            with patch.object(agent._validator, "validate_plan", new_callable=AsyncMock) as mock_val:
                ...
                with patch.object(agent._validator, "plan_recovery", new_callable=AsyncMock) as mock_rec:
                    ...
                    mock_rec.return_value = RecoveryPlan(
                        ...
                        modified_step=PlanStep(id="s1", command="exploit", tool="metasploit", args={"retry": True}),
...
async def test_e2e_simulation_swarm_routing():
    ...
    step1 = PlanStep(id="s1", command="Check database", tool="_subagent", args={"role": "database_specialist", "goal": "Audit DB on 10.0.0.3"})
    plan.steps = [step1]
    ...
    with patch.object(agent._planner_autonomous, "plan", new_callable=AsyncMock) as mock_plan:
        ...
        with patch.object(agent._executor_autonomous, "execute_plan", side_effect=mock_execute_plan):
            with patch.object(agent._validator, "validate_plan", new_callable=AsyncMock) as mock_val:
                ...
                with patch.object(agent, "execute_subagent", new_callable=AsyncMock) as mock_subagent:
                    ...
                    assert result.success is True
                    assert result.plan.steps[0].status == StepStatus.COMPLETED
                    mock_subagent.assert_called_once_with("database_specialist", "Audit DB on 10.0.0.3")
