...
def main():
    ...
    tests = [
        ...
        # ═══════════════════════════════════════════════════════════════
        ("find tech stack then scan for vulnerabilities on https://example.com", None, 2, None, None, None),
        ("enumerate subdomains then scan for open ports on example.com", None, 2, None, None, None),
        ("check headers then test cors then audit ssl on https://example.com", None, 3, None, None, None),
        ...
        ("run wpscan && then brute force admin login on https://wordpress-site.com", None, 2, None, None, None),
        ("check for exposed git and then enumerate api endpoints on https://example.com", None, 2, None, None, None),
        ("find all subdomains && scan for heartbleed && check cve-2024-1234 on example.com", None, 3, None, None, None),
        ...
        ("check for log4j heartbleed and shellshock on https://example.com", ["nuclei", "nikto"], 2, None, None, None),
        ("list s3 buckets and check iam roles on https://aws.example.com", ["curl"], 1, None, None, None),
        ("find exposed panels and check for default creds on https://example.com", ["nuclei"], 1, None, None, None),
        ...
        ("audit web app without directory bruteforce on https://example.com", None, 1, None, None, None),
        ...
        ("brute force login page with hydra on https://example.com/login", ["hydra", "nmap"], 2, None, None, None),
        ("scan 10.0.0.0/24 using rustscan", None, 1, None, None, None),

        # ═══════════════════════════════════════════════════════════════
        # SECTION 5: ADVANCED WEB VULNERABILITY DETECTION
        # ═══════════════════════════════════════════════════════════════
        ("check for ssrf vulnerability on https://api.example.com", ["nuclei"], 1, None, None, None),
        ("test for idor and broken access control on https://example.com", ["nuclei"], 1, None, None, None),
        ("check for lfi rfi vulnerabilities on https://example.com", ["nuclei"], 1, None, None, None),
        ("scan for open redirect and clickjacking on https://example.com", ["curl", "nuclei"], 2, None, None, None),
        ("check for insecure deserialization on https://example.com", ["nuclei"], 1, None, None, None),
        ("test websocket security on wss://example.com/socket", ["curl"], 1, None, None, None),
        ("graphql introspection query on https://api.example.com/graphql", ["curl"], 1, None, None, None),
        ("check swagger api documentation exposure on https://api.example.com", ["curl"], 1, None, None, None),
        ...
        ("kerberoast all users on domain controller", ["impacket-GetUserSPNs"], 1, None, None, None),
        ("asrep roast domain users on dc.example.com", ["impacket-GetNPUsers"], 1, None, None, None),
        ...
        ("audit azure blob storage containers on https://storage.blob.core.windows.net", ["curl"], 1, None, None, None),
        ("enumerate gcp cloud storage buckets on https://storage.googleapis.com", ["curl"], 1, None, None, None),
        ("check for cloudfront misconfiguration on https://d1.example.cloudfront.net", ["curl"], 1, None, None, None),
        ...
        ("check container registry for exposed images on 10.0.0.1:5000", ["curl"], 1, None, None, None),
        ...
        ("scan example.com and scan example.org and scan example.net", ["nmap"], 1, None, None, None),  # Multi-target
        ("check http://example.com and https://example.org", ["curl"], 1, None, None, None),  # URLs
        ("perform a thorough deep dive security assessment of the external attack surface for example.com", None, 1, None, None, None),
    ]
