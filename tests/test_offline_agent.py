...
async def test_pillar_1_conditional_interpreter_and_evaluator():
    ...
    task_cond = interpreter.interpret("if port_80_open then scan 192.168.1.1 with nikto else scan 192.168.1.1 with nmap")
...
async def test_pillar_3_adaptive_plan_mutator():
    ...
    step_nmap = ExecutionStep(id="step_1", step_type=StepType.TOOL_RUN, tool="nmap", args=[], target="192.168.1.100")
    sr_failed = StepResult(step_id="step_1", status=StepStatus.FAILED, error="Host seems down. If it is really up, try -Pn")
    still_pending = []
    ...
    step_gobuster = ExecutionStep(id="step_2", step_type=StepType.TOOL_RUN, tool="gobuster", args=[], target="192.168.1.100")
    ...
    sr_permission = StepResult(step_id="step_3", status=StepStatus.FAILED, error="Permission denied")
    still_pending_shell = []
