...
if command -v ohpm &>/dev/null; then
  ...
  ohpm install @mufthakherul/siyarix@"^${SIYARIX_VERSION}" && INSTALLED=1
fi

# Check for hpm (legacy HarmonyOS package manager)
if [ "$INSTALLED" -eq 0 ] && command -v hpm &>/dev/null; then
  echo "==> Installing via hpm..."
  hpm install -g @mufthakherul/siyarix && INSTALLED=1
fi

# Fallback: pip
if [ "$INSTALLED" -eq 0 ] && command -v python3 &>/dev/null; then
  PY_VER=$(python3 --version 2>&1 | grep -oP '\d+\.\d+' | head -1)
  PY_MAJ=${PY_VER%.*}
  PY_MIN=${PY_VER#*.}
  if [ "$PY_MAJ" -ge 3 ] && [ "$PY_MIN" -ge 11 ]; then
    echo "==> Installing via pip (fallback)..."
    python3 -m pip install siyarix 2>/dev/null || python3 -m pip install --user siyarix && INSTALLED=1
  fi
fi

if [ "$INSTALLED" -eq 0 ]; then
  echo "==> WARNING: Could not install via any method."
  echo "    Install Python 3.11+ for HarmonyOS from:"
