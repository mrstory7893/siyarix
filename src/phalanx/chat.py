...
class SiyarixChat:
    ...
    async def _cmd_collab(self, args: str) -> None:
        ...
        if action == "create":
            ...
            if len(tokens) < 2:
                ...
                return
            console.print("[yellow]SSH collaboration requires the Siyarix SSH gateway service running on the host.[/yellow]")
            console.print(f"[dim]To connect: ssh siyarix@{socket.gethostname()} -- session {tokens[1]}[/dim]")
    ...
    async def _cmd_team(self, args: str) -> None:
        ...
        if action == "list":
            ...
            console.print("[yellow]For multi-user collaboration, use: siyarix team invite (CLI)[/yellow]")
        elif action == "invite":
            member = tokens[1] if len(tokens) > 1 else Prompt.ask("Member name/email")
            console.print(f"[green]✓ Invitation sent to {member}[/green]")
            console.print("[yellow]The invited member can join with: siyarix team --session <name>[/yellow]")
        elif action == "roles":
            engine = PersonaEngine()
