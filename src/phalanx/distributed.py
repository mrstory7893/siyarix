...
multiple Siyarix instances can share work. Moves the SQLite offline store
...
class TaskQueueBackend:
    ...
    async def _enqueue_redis(self, task: DistributedTask) -> str:
        try:
            ...
            await r.rpush("siyarix:queue", json.dumps(task.__dict__, default=str))
            return task.task_id
        except ImportError:
    ...
    async def _dequeue_redis(self, worker_id: str) -> DistributedTask | None:
        try:
            r = self._get_redis_client()
            data = await r.blpop("siyarix:queue", timeout=5)
            if data:
