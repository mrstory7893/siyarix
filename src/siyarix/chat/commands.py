...
_BUILTIN_COMMANDS: list[CommandInfo] = [
    ...
    CommandInfo(
        name="/undo",
        category=CommandCategory.NAVIGATION,
        description="Rollback the last turn of conversation",
        usage="/undo [steps]",
        aliases=["/rollback"],
        args=[ArgInfo("steps", "Number of conversation turns to undo (default: 1)", optional=True)],
        handler="_cmd_undo",
        examples=["/undo", "/undo 3"],
        notes="Removes user query and assistant response for the last N turns.",
    ),
    CommandInfo(
        name="/retry",
        category=CommandCategory.NAVIGATION,
        ...
        handler="_cmd_retry",
        examples=["/retry"],
        notes="Pops the last assistant response and re-executes the last user message.",
    ),
    CommandInfo(
        name="/edit",
        category=CommandCategory.NAVIGATION,
        description="Edit the last user prompt and re-execute",
        usage="/edit <new prompt>",
        args=[ArgInfo("new_prompt", "The new query/instruction")],
        handler="_cmd_edit",
        examples=["/edit scan target.com --ports 80,443"],
        notes="Replaces the last user prompt with the new one and executes it.",
    ),
    CommandInfo(
        name="/history",
        category=CommandCategory.NAVIGATION,
