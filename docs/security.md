...
The `CredentialStore` (accessible via `siyarix auth`) is a secure vault for API keys (e.g., OpenAI, Gemini) and tool credentials.

- **Encryption**: All secrets are encrypted at rest using **Fernet symmetric encryption**.
- **Envelope Encryption (Optional)**: NexSec supports AWS KMS for envelope encryption. When enabled (`SIYARIX_KMS_PROVIDER=aws`), the local Fernet key is protected by an AWS KMS master key.
...
NexSec maintains a local enterprise audit trail (`~/.siyarix/audit.json`) that records every significant action taken by the agent.
...
- **Verification**: Run `siyarix audit verify` to ensure the audit log has not been tampered with.
- **Compliance Export**: Supports SOC2, ISO 27001, and NIST report generation via `siyarix audit report`.
...
- **CI/CD Policy Gates**: Use `siyarix ci gate` in your pipelines to automatically fail builds if critical vulnerabilities are detected or if the system health is compromised.

## Best Practices

1. **Rotate Keys**: Use `siyarix auth set-key` to rotate API keys periodically.
