# Getting Started with Siyarix

Hey there! Setting up Siyarix is designed to be straightforward. Because the core engine is built in Python, you can run it natively on almost any operating system (Linux, macOS, or Windows). 
...
- **Python 3.11 or higher**: Siyarix relies on modern Python features (like `asyncio` and advanced type hinting).
...
- Have a few security tools installed on your `PATH` (e.g., `nmap`, `nuclei`, `ffuf`). Siyarix acts as a brain that orchestrates these tools, so the more tools you have installed natively, the more powerful Siyarix becomes!

---

## 📦 Installation Guide

We strongly recommend installing Siyarix inside an isolated virtual environment so it doesn't conflict with your system Python packages.
...
```bash
pip install "siyarix[autonomous,cli]"
```
...
Because Siyarix uses AI models to help plan your tasks, you'll need an API key for your preferred LLM provider (like Google Gemini, OpenAI, or Anthropic). You can also use Ollama for local, offline models!

Don't worry, your keys are stored securely in a local encrypted vault (`~/.siyarix/`). They are never sent anywhere without your permission.
