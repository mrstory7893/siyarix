...
Navigate to `src/siyarix/parsers/` and create a new file called `sec_scanner.py`.
...
```python
from siyarix.parsers.base import BaseParser
...
### 4. Test It!
Run `siyarix` and ask it to use your tool:
```bash
siyarix run "execute sec-scanner on example.com"
```
If your parser works, Siyarix will successfully parse the output into a structured table and the AI will be able to summarize the `findings_count` accurately!
