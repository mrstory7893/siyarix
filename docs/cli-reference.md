...
This document provides a comprehensive reference for the NexSec command hierarchy. The CLI is built with Typer; use `siyarix --help` or `siyarix <subcommand> --help` for real-time usage and flag details.

> Launching `siyarix` with no subcommand opens the interactive assistant shell. The landing screen shows your platform, current mode, theme, model provider, session ID, and quick actions for the most common tasks.
...
- **Example**: `siyarix run "find open ports on 10.0.0.1"`
